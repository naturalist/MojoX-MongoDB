...
eval {
    ...
    $m = MojoX::MongoDB::Connection->new(host => $host);
};
...
is($file->{'chunkSize'}, $MojoX::MongoDB::GridFS::chunk_size);
...
isa_ok($file, 'MojoX::MongoDB::GridFS::File');
...
for (my $i=0; $i<3; $i++) {
    isa_ok($list[$i], 'MojoX::MongoDB::GridFS::File');
}
