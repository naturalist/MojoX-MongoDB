...
# id realloc
{
    ...
    my $med_str = "z" x 4014;
    $c->insert({'text' => $med_str, 'id2' => MojoX::MongoDB::OID->new});
    my $result = $c->find_one;
...

{
    ...
    my $id = $c->insert({"n" => undef,
                ...
                "a" => {"foo" => "bar",
                        "n" => undef,
                        "x" => MojoX::MongoDB::OID->new("49b6d9fb17330414a0c63102")},
                "d2" => DateTime->from_epoch(epoch => 1271079861),
                "regex" => qr/xtz/,
                "_id" => MojoX::MongoDB::OID->new("49b6d9fb17330414a0c63101"),
                "string" => "string"});
    ...
    is($obj->{'a'}->{'foo'}, 'bar');
    is($obj->{'a'}->{'n'}, undef);
    isa_ok($obj->{'a'}->{'x'}, 'MojoX::MongoDB::OID');
    isa_ok($obj->{'d2'}, 'MojoX::MongoDB::DateTime');
    is($obj->{'d2'}->epoch, 1271079861);
    ok($obj->{'regex'});
    isa_ok($obj->{'_id'}, 'MojoX::MongoDB::OID');
    is($obj->{'_id'}, $id);
    is($obj->{'string'}, 'string');
}

{
    $MojoX::MongoDB::BSON::char = "=";
    $c->drop;
...
{
    $MojoX::MongoDB::BSON::looks_like_number = 1;
    ...
    $MojoX::MongoDB::BSON::looks_like_number = 0;
}

# MojoX::MongoDB::BSON::String type
{
    $MojoX::MongoDB::BSON::looks_like_number = 1;
    ...
    $c->insert({num => bless(\$num, "MojoX::MongoDB::BSON::String")}, {safe => 1});
